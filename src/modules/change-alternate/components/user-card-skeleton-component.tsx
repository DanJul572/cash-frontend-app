  ...
  return Array.from({ length: SKELETON_COUNT }).map((_, index) => (
    <Card key={index} sx={userCardSkeletonComponentStyle.cardStyle}>
      <CardContent sx={userCardSkeletonComponentStyle.cardContentStyle}>
        <Skeleton variant="circular" sx={userCardSkeletonComponentStyle.avatarSkeletonStyle} />
        <Skeleton variant="rectangular" sx={userCardSkeletonComponentStyle.buttonSkeletonStyle} />
      </CardContent>
    </Card>
  ));
}
